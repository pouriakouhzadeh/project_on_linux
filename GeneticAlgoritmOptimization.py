...
class GeneticAlgorithm:
    def __init__(self):
        ...
        param_space = [(6, 10), (1, 20), (30, 800), (2000, 6999), (1000, 3000), (52, 75), (8, 18)]
    ...
    def main(self, currency_pair, pop, NG):
        ...
        for i, ind in enumerate(best_individuals, 1):
            ...
            if ind.fitness.values[0] > 0.7 :
                currency_data = self.read_data(f"{currency_pair}.csv", 7000)
                if self.CV.Train(self.df_to_json(currency_data.copy()), *ind[:-1], ind[-1]) > (0.7, ) :
                    if self.TR.Train(self.df_to_json(currency_data.copy()), *ind[:-1], ind[-1]) > (0.7, ) :
        ...
        print(f"Model for {currency_pair} saving failed")
        return False
