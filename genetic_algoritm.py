...
def model_prediction(population, currency_files):
    total_acc = 0
    valid_models = 0
    tasks = []
    ...
    for task in results:
            try:
                acc, wins, loses = task  # اینجا برای هر کار منتظر می‌مانیم تا تکمیل شود
    ...
    return avg_acc / valid_models if valid_models > 0 else 0
    return avg_acc
...
def genetic_algorithm_for_all_currencies(currency_files, population_size, generations, mutation_rate, param_space, results_file="ga_results.txt"):
    ...
    for currency_file in currency_files:
        ...
        for generation in range(generations):
            fitness_scores = [model_prediction (population, [currency_file]) ]
...
param_space = [(2, 4), (2, 20), (30, 500), (500, 6000), (100, 500), (52, 70), (3, 22)]

genetic_algorithm_for_all_currencies(currency_files, population_size=10, generations=3, mutation_rate=0.2, param_space=param_space)
